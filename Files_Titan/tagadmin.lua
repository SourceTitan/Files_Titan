...
local function keko_Titan(data)
    ...
    local vip = database:sismember('Titan:'..bot_id..'vipgp:'..chat_id, user_id)
    ...
    -----------------------------
    if text and text == "تفعيل تاك الادمنيه" and database:sismember('Titan:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then 
    database:set("Titan:tag:admin:"..bot_id..msg.chat_id_,"Titan")
    send(msg.chat_id_, msg.id_, 1, "⚠️┇تم تفعيل تاك الادمنيه", 1, 'html')
    end
    if text and text == "تعطيل تاك الادمنيه" and database:sismember('Titan:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then 
    database:del("Titan:tag:admin:"..bot_id..msg.chat_id_)
    send(msg.chat_id_, msg.id_, 1, "⚠️┇تم تعطيل تاك الادمنيه", 1, 'html')
    end
    if (text and ( text == 'صيح الادمنيه' or text == "تاك للادمنيه" or text == "وين الادمنيه") and (is_vip(msg) or database:get("Titan:tag:admin:"..bot_id..msg.chat_id_))) then 
    function cb(t1,t2)
    ...
    i = i + 1
    local user_info = database:hgetall('Titan:'..bot_id..'user:'..v.user_id_)
    ...
    send(msg.chat_id_, msg.id_, 1, new_text, 1, 'Titan')
    end
