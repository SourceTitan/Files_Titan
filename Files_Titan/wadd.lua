...
local function keko_Titan(data)
    ...
    database:set("Titan:loock:add:w:"..msg.chat_id_..bot_id,"Titan")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    if text and text == "تفعيل ضافني" then 
    database:del("Titan:loock:add:w:"..msg.chat_id_..bot_id)
    send(msg.chat_id_, msg.id_, 1, "☑️┇ تم تفعيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    end
    if not database:get("Titan:loock:add:w:"..msg.chat_id_..bot_id) then 
    if msg.content_.ID == "MessageChatAddMembers" then
    database:set("Titan:add:me:w:"..bot_id..msg.chat_id_..msg.content_.members_[0].id_,msg.sender_user_id_)    
    end 
    if text and text:match("(.*)(ضافني)(.*)") then 
    if_keko =  database:get("Titan:add:me:w:"..bot_id..msg.chat_id_..msg.sender_user_id_)    
    if not if_keko then 
